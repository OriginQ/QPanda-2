...
namespace QPanda
{
    ...
    void OriginSLSQP::set_lower_and_upper_bounds(vector_d& lower_bound, vector_d& upper)
    {
        nlopt_set_lower_bounds(&opter, lower_bound.data());
        nlopt_set_upper_bounds(&opter, upper.data());
    }

    void OriginSLSQP::add_equality_constraint(QOptFunc func)
    {
    ...
    void OriginSLSQP::add_inequality_constraint(QOptFunc func)
    {
        nlopt_func a([func](unsigned n, const double* x, double* gradient, void* func_data)
            {
                vector_d x_data(x, x + n), grad_data(gradient, gradient + n);
    ...
    void OriginSLSQP::init() {
        ...
        opter = nlopt_create(nlopt_algorithm::NLOPT_LD_SLSQP, m_dimension);
