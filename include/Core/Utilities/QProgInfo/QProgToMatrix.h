...
class QProgToMatrix
{
	...
	using calcUintItem_t = std::pair<QMatrixXcd, std::vector<int>>;
	...
	protected:
		void reverse_ctrl_gate_matrix(QMatrixXcd& src_mat, const GateType &gate_T);
		QMatrixXcd reverse_ctrl_gate_matrix_CX(QMatrixXcd& src_mat);
		QMatrixXcd reverse_ctrl_gate_matrix_CU(QMatrixXcd& src_mat);
		void merge_to_calc_unit(std::vector<int>& qubits, QMatrixXcd& gate_mat, calcUnitVec_t &calc_unit_vec, gateQubitInfo_t &single_qubit_gates);
		void get_stride_over_qubits(const std::vector<int> &qgate_used_qubits, std::vector<int> &stride_over_qubits);
		void tensor_by_matrix(QMatrixXcd& src_mat, const QMatrixXcd& tensor_mat);
		void tensor_by_QGate(QMatrixXcd& src_mat, std::shared_ptr<AbstractQGateNode> &pGate);
		...
		void build_standard_control_gate_matrix(const QMatrixXcd& src_mat, const int qubit_number, QMatrixXcd& result_mat);
		void swap_two_qubit_on_matrix(QMatrixXcd& src_mat, const int mat_qubit_start, const int mat_qubit_end, const int qubit_1, const int qubit_2);
	...
	public:
		...
		QMatrixXcd m_current_layer_mat;
...
public:
	...
	* @return QMatrixXcd the matrix of the layer
	*/
	QMatrixXcd get_matrix_of_one_layer(SeqLayer<pOptimizerNodeInfo>& layer);
