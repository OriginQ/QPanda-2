...
void init_core_class(py::module & m)
{
    ...
    py::class_<OriginQubitPool, std::unique_ptr<OriginQubitPool, py::nodelete>>(m, "OriginQubitPool")
        ...
        .def("qAlloc_many", [](OriginQubitPool& self, size_t num) { std::vector<Qubit*> temp = self.qAllocMany(num); return temp; }, \
	...
	*/
	...
	py::class_<Fusion>(m, "Fusion")
		.def(py::init<>())
		.def("aggregate_operations", [](Fusion& self, QCircuit& cir, QuantumMachine* qvm) {
			return self.aggregate_operations(cir, qvm); },
			"cir"_a," QuantumMachine"_a,
			py::return_value_policy::automatic)

		.def("aggregate_operations", [](Fusion& self, QProg& prog, QuantumMachine* qvm) {
