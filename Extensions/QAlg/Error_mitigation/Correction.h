...
#include "QAlg/Error_mitigation/Sample.h"
...
#ifdef USE_CHIP
    Mitigation(const QVec& q, QPilotOSMachine* qvm, size_t shots);
#endif // USE_CHIP


    void readout_error_mitigation(const sample_method &method, const std::vector<double>& m_unmiti_prob);
    std::vector<std::vector<double>> zne_circuit(QProg& prog, const std::vector<std::tuple<int, int, int>>& amplify_factors, bool random = false, bool detail = false);
    QCircuit get_prog_from_layer(const LayeredTopoSeq& prog_seq, size_t layer_start, size_t layer_end);
    QProg remove_measure(QProg& prog);
    QProg add_measure(QProg& prog);
    size_t state_dim();
    void zne_error_mitigation(const std::vector<double>& order, const std::vector < std::vector<double>>& error_result);
    QProg clifford_replace(QProg &original_prog);
...
#ifdef USE_CHIP
    QPilotOSMachine* m_real_chip;
#endif // USE_CHIP
    NoiseModel m_noise;
    std::vector<double> m_miti_prob;
    std::vector<std::shared_ptr<AbstractQuantumMeasure>> m_measure_node;
    std::vector<std::pair<uint32_t, CBit*>> m_measure_info;
    Sample m_sample;
};
#endif
