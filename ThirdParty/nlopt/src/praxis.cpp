...
/*     MACHINE DEPENDENT NUMBERS: */
	...
	work = (double*)malloc(sizeof(double) * ((int64_t)n * n + (int64_t)n * 9));
    if (!work) return nlopt_result::NLOPT_OUT_OF_MEMORY;
    q_1.v = work;
	q_1.q0 = q_1.v + (int64_t)n * n;
    q_1.q1 = q_1.q0 + n;
    q_1.t_flin = q_1.q1 + n;
...
/*     FX MUST BE PASSED TO MIN BY VALUE. */
    ...
    if (ret != nlopt_result::NLOPT_SUCCESS) goto done;
    if (s > 0.) {
	goto L50;
    }
    i__1 = n;
...
/* .....RETURN..... */
    ...
    if (ret != nlopt_result::NLOPT_OUT_OF_MEMORY) {
	 *minf = q_1.fbest;
...
L4:
    ...
    else if (ret_val <= stop->minf_max) *ret = nlopt_result::NLOPT_MINF_MAX_REACHED;
    return ret_val;
} /* flin_ */
