...
public:
    ...
    QuantumVolume(QCloudTaskConfig config);

	QuantumVolume(MeasureQVMType type, QuantumMachine* qvm);
	~QuantumVolume();

	size_t calcQuantumVolume(const std::vector<std::vector<int>> &qubit_lists, int ntrials, int shots, QMachineType type = QMachineType::NOISE);
...
private:
    ...
    void createQvCloudCircuits(std::vector<std::vector<int> > qubit_lists,
        int ntrials,
    ...
    void calcIRealChipResult(std::vector<std::vector <QvCircuit> >&circuits, int shots);
...
size_t calculate_quantum_volume(QCloudTaskConfig config, std::vector <std::vector<int> >qubit_lists, int ntrials);
