...
static void get_actual_matrix(const QGate& gate_in, const QGate &other_in,
    QStat& matrix1_out, QStat& matrix2_out)
{
    gate_in.getQGate()->getMatrix(matrix1_out);
    other_in.getQGate()->getMatrix(matrix2_out);

    if (matrix1_out.size() != matrix2_out.size())
        QCERR_AND_THROW(run_fail, "matrix dimension mismatch");

    if (gate_in.isDagger())
        dagger(matrix1_out);

    if (other_in.isDagger())
        dagger(matrix2_out);

    return;
}

static QVec assert_vectors_equal(const QGate &gate, const QGate &other)
{
    QVec qubits;
    gate.getQuBitVector(qubits);

    QVec other_qubits;
    other.getQuBitVector(other_qubits);

    Qnum vec1, vec2;
    for (auto val : qubits)
        vec1.emplace_back(val->get_phy_addr());

    for (auto val : other_qubits)
        vec2.emplace_back(val->get_phy_addr());

    if (vec1.size() != vec2.size())
        QCERR_AND_THROW(run_fail, "qubits addr mismatch");
...
QGate QGate::exp(double exponent)
{
    QStat gate_matrix;
    getQGate()->getMatrix(gate_matrix);

    if (isDagger())
        QPanda::dagger(gate_matrix);

    QVec qubits;
    getQuBitVector(qubits);

    auto n = std::sqrt(gate_matrix.size());

    Eigen::MatrixXcd matrix = Eigen::MatrixXcd::Zero(n, n);
    for (auto rdx = 0; rdx < n; ++rdx)
        for (auto cdx = 0; cdx < n; ++cdx)
            matrix(rdx, cdx) = gate_matrix[rdx * n + cdx];

    Eigen::MatrixXcd exp_matrix = matrix.pow(exponent);

    QStat state_matrix(exp_matrix.data(), exp_matrix.data() + exp_matrix.size());

    return QOracle(qubits, state_matrix);
}

QGate QGate::operator*(const QGate &other)
{
    QStat this_matrix, other_matrix;
    get_actual_matrix(*this, other, this_matrix, other_matrix);
    auto qubits = assert_vectors_equal(*this, other);
    return QOracle(qubits, this_matrix * other_matrix);
}
...
OriginQGate::OriginQGate(QVec& qubit_vector, QuantumGate* qgate) :m_Is_dagger(false)
{
    ...
    if (_check_duplicate(qubit_vector))
    {
        QCERR_AND_THROW_ERRSTR(run_fail, "duplicate qubits.")
    }
...
Qubit* OriginQGate::popBackQuBit()
{
    ...
    auto iter = m_check_qubits.find(temp->get_phy_addr());
    m_check_qubits.erase(iter);
...
bool OriginQGate::_check_duplicate(const QVec &add_qubits)
{
    for (auto &q : add_qubits) {
        if (!m_check_qubits.insert(q->get_phy_addr()).second) {
            return true;
        }
    }
...
void OriginQGate::clear_control()
{
    for (size_t i = 0; i < m_control_qubit_vector.size(); i++)
    {
        size_t src_addr = m_control_qubit_vector[i]->get_phy_addr();
        auto iter = m_check_qubits.find(src_addr);
        m_check_qubits.erase(iter);
    }
...
void OriginQGate::clear_qubits()
{
    m_qubit_vector.clear();
    m_control_qubit_vector.clear();
    m_check_qubits.clear();
...
{
    for (size_t i = 0; i < m_qubit_vector.size(); i++)
    {
        size_t src_addr = m_qubit_vector[i]->get_phy_addr();
        auto iter = m_check_qubits.find(src_addr);
        m_check_qubits.erase(iter);
    }
    ...
    if (_check_duplicate(qubit_vector))
    {
        QCERR_AND_THROW_ERRSTR(run_fail, "remap duplicate qubits.")
    }
...
QGate QPanda::QOracle(const QVec &qubits, const QStat &matrix, const double tolerance)
{
    if (!is_unitary_matrix_by_eigen(matrix, tolerance))
    {
...
QGate  QPanda::MS(Qubit* control_qubit, Qubit* target_qubit)
{
    string name = "MS";
    if (control_qubit == target_qubit)
    {
...
QCircuit  QPanda::MS(const QVec &control_qubits, const QVec &target_qubits)
{
    if (control_qubits.size() == 0 || target_qubits.size() == 0)
    {
        QCERR("qubit_vector err");
...
QGate QPanda::MS(int control_qaddr, int target_qaddr)
{
    if (control_qaddr == target_qaddr)
    {
        QCERR(" target bit is the same as the control bit");
    ...
    return MS(get_qubit_by_phyaddr(control_qaddr), get_qubit_by_phyaddr(target_qaddr));
}

QCircuit QPanda::MS(const std::vector<int>& control_qaddrs, const std::vector<int>& target_qaddrs)
{
    if (control_qaddrs.size() == 0 || target_qaddrs.size() == 0)
    {
        QCERR("qubit_vector err");
    ...
    if (control_qaddrs.size() == target_qaddrs.size())
    {
        for (int i = 0; i < control_qaddrs.size(); ++i)
        {
            if (control_qaddrs[i] != target_qaddrs[i])
            {
                cir << MS(get_qubit_by_phyaddr(control_qaddrs[i]), get_qubit_by_phyaddr(target_qaddrs[i]));
            }
            else
            {
                QCERR("double_gate qubit err");
