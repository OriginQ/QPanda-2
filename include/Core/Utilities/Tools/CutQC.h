...
public:
	...
	using smatrix_t = Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>;
	...
	*/
    ...
    using ChoiMatrices = std::map<std::string, cmatrix_t>;
...
protected:
	...
	cmatrix_t target_labels_to_matrix(const ResultData& data);
	...
	std::vector<cmatrix_t> target_labels_to_matrix(const std::vector<FragLabel>& labels);
