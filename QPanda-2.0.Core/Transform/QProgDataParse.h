...
class QProgDataParse
{
    ...
    typedef list<pair<uint_t, DataNode>>        dataList_t;
    typedef map<int, string>                    gateMap_t;
    typedef map<int, string>                    operatorMap_t;
public:
    QProgDataParse(const string &filename);
    ~QProgDataParse();
...
protected:
    ...
    */
    ...
    Note:
        None
    */
    float getAngle(const pair<uint_t, DataNode> &data);
private:
    string m_filename;
    ...
    list<pair<uint_t, DataNode>>::iterator m_iter;
    stack<ClassicalCondition> m_stack_cc;
};
