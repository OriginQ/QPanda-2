...
string QPanda::draw_qprog(QProg prog, PIC_TYPE p /* = PIC_TYPE::TEXT */, uint32_t length /*= 100*/, const std::string &output_file /*= ""*/,
						  const NodeIter itr_start /* = NodeIter()*/, const NodeIter itr_end /* = NodeIter()*/)
{
	...
	return test_text_pic.textDraw(LAYER, p, length);
}

std::string QPanda::draw_qprog(QProg prog, LayeredTopoSeq &m_layer_info, PIC_TYPE p /* = PIC_TYPE::TEXT */, uint32_t length /*= 100*/,
...
std::string QPanda::draw_qprog_with_clock(QProg prog,  PIC_TYPE p/* = PIC_TYPE::TEXT */, const std::string config_data /*= CONFIG_PATH*/,
										  uint32_t length /*= 100*/, const std::string &output_file /*= ""*/, const NodeIter itr_start /* = NodeIter()*/, const NodeIter itr_end /*= NodeIter()*/)
{
	DrawQProg test_text_pic(prog, itr_start, itr_end, output_file);
	return test_text_pic.textDraw(TIME_SEQUENCE, p, length, config_data);
}
