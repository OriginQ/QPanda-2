...
TEST(QVM, TotalAmplitudeQVM)
{
    ...
    auto qlist = machine->allocateQubits(16);
    auto clist = machine->allocateCBits(16);
    ...
    transformQRunesToQProg("D:\\QRunes1818.txt", prog, machine);
    ...
    for (int i = 0; i < 64; ++i)
    {
        cout << i << " : " << res[i] <<" prob: "<< getStateProb(res[i])<< endl;
    }
}

TEST(QVM, TinyTotalAmplitudeQVM)
{
    throw exception();
    auto machine = initQuantumMachine(CPU_SINGLE_THREAD);
    auto qlist = machine->allocateQubits(8);
    auto clist = machine->allocateCBits(8);
...
TEST(QVM, PartialAmplitudeQVM)
{
    ...
    //cout << res.find("00000000000")->first << " : "
    //     << res.find("00000000000")->second << endl;
    //cout << res.find("00000000001")->first << " : "
    //     << res.find("00000000001")->second << endl;
    //cout << res.find("00000000010")->first << " : "
    //     << res.find("00000000010")->second << endl;
    //cout << res.find("00000000011")->first << " : "
    //     << res.find("00000000011")->second << endl;
    //cout << res.find("00000000100")->first << " : "
    //     << res.find("00000000100")->second << endl;
    //cout << res.find("00000000101")->first << " : "
    //     << res.find("00000000101")->second << endl;

    cout << "---------------------" << endl;

    auto res1 = machine->PMeasure("64");
    for (auto val : res1)
    {
    ...
    ////cout << machine->PMeasure_bin_index("0000000000") << endl;
...
TEST(QVM, SingleAmplitudeQVM)
{
    ...
    machine->init();
    auto qlist = machine->allocateQubits(72);
    auto clist = machine->allocateCBits(72);
