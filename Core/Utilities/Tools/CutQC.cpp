...
using svector_t = Eigen::Matrix<double, Eigen::Dynamic, 1>;
...
RecombineFragment::cmatrix_t RecombineFragment::target_labels_to_matrix(const ResultData& data)
{
	const auto& prep_labels = data.m_prep_labels;
	const auto& meas_labels = data.m_meas_labels;

	cmatrix_t prep_matrix = cmatrix_t::Identity(1, 1);
	cmatrix_t meas_matrix = cmatrix_t::Identity(1, 1);
	...
	for (auto label : prep_labels)
	{
		...
		prep_matrix = Eigen::kroneckerProduct(cmatrix_t::Map(&label_matrix[0], 2, 2), prep_matrix).eval();
	...
	for (auto label : meas_labels)
	{
		...
		meas_matrix = Eigen::kroneckerProduct(cmatrix_t::Map(&label_matrix[0], 2, 2), meas_matrix).eval();
...
void RecombineFragment::get_choi_matrix(const ResultDataMap& organize_data, ChoiMatrices& choi_states)
{
	for (const auto& data : organize_data)
	{
        ...
        cmatrix_t state_matrix = cmatrix_t::Zero(result_datas.size() + 1, pow(2, 2*cut_qubit_num));
		...
		for (auto i = 0; i < prep_meas_states.size(); ++i)
		{
			...
			state_matrix.row(i) = cvector_t::Map(label_matrix.data(), label_matrix.size());
		}

		cmatrix_t identity_matrix = cmatrix_t::Identity(1ull << cut_qubit_num, 1ull << cut_qubit_num);
		state_matrix.row(result_datas.size()) = cvector_t::Map(identity_matrix.data(), identity_matrix.size());


		cvector_t state_counts = svector_t::Map(counts.data(), counts.size()).cast<qcomplex_t>();
        /*PTrace("state_matrix \n" << state_matrix);
        PTrace("state_counts \n" << state_counts);*/
		...
		//cmatrix_t choi_fit = state_matrix.conjugate().bdcSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(state_counts);
        //cmatrix_t choi_fit = state_matrix.conjugate().colPivHouseholderQr().solve(state_counts); /**< method 2 */
        //cmatrix_t choi_fit = state_matrix.conjugate().householderQr().solve(state_counts); /**< method 3 */
        //cmatrix_t choi_fit = state_matrix.conjugate().jacobiSvd(Eigen::ComputeThinU | Eigen::ComputeThinV).solve(state_counts); /**< method 4 */
		
		/* 速度较快的线性求解方法求解方法
		*/
		LeastSquaresConjugateGradient <cmatrix_t> solver;
		//ConjugateGradient  <cmatrix_t> solver;
		solver.compute(state_matrix.conjugate());
		cmatrix_t choi_fit = solver.solve(state_counts);
 
		//PTrace("choi_fit" << choi_fit);
        //PTrace("choi_fit");

        Eigen::Map<cmatrix_t> reshape_matrix(choi_fit.data(), 1ull << cut_qubit_num, 1ull << cut_qubit_num);
...
std::vector<cmatrix_t> RecombineFragment::target_labels_to_matrix(const std::vector<FragLabel>& labels)
{
	auto string_to_matrix_fun = [&](const std::string& _str) ->cmatrix_t {
		if (_str.size() == 0) {
			return cmatrix_t::Identity(1, 1);
		}

		if (_str.compare("I") == 0){
			return cmatrix_t::Identity(2, 2);
		}
		...
		case 'X':
		{
			...
			return cmatrix_t::Map(&label_matrix_x[0], 2, 2);
		}
		break;
		...
		case 'Y':
		{
			...
			return cmatrix_t::Map(&label_matrix_y[0], 2, 2);
		}
		break;
		...
		case 'Z':
		{
			...
			return cmatrix_t::Map(&label_matrix_z[0], 2, 2);
		}
		break;
	...
	auto label_to_matrix_fun = [&](const std::string& _tmp_label) ->cmatrix_t {
		cmatrix_t _label_matrix = cmatrix_t::Identity(1, 1);
	...
	std::vector<cmatrix_t> frag_mats;
	...
	for (const auto& _lables : combine_lables)
	{
		...
		*/
		...
		for (const auto& _joined_str : joined_str_vec)
		{
			...
			std::vector<EigenComplexT> val_vec(_joined_str.size(), 0);
			for (size_t _i = 0; _i < _joined_str.size(); ++_i)
			{
				...
				const cvector_t _mat = cvector_t::Map(_t_frag_mats.eval().data(), _t_frag_mats.size());
				//PTrace("frag_mats: " << _sub_cir_index << ":\n" << _mat);

				const cvector_t _choi = cvector_t::Map(choi_states_vec[_sub_cir_index].at(_joined_str[_i]).data(),
...
static cmatrix_t array_outer(cvector_t choi_vector)
{
	auto dim = choi_vector.size();

	cmatrix_t matrix = cmatrix_t::Zero(dim, dim);

	for (auto i = 0; i < dim; ++i)
	{
		cvector_t outer_result = choi_vector.conjugate()[i] * choi_vector;
	...
	std::vector<ChoiMatrices>& likely_choi_states_vec)
{
	...
	{
		std::map<string, svector_t> choi_eigs;
		std::map<string, cmatrix_t> choi_vecs;
		for (auto val : choi_state)
		{
			auto final_bits = val.first;
			auto choi_state = val.second;

			cmatrix_t choi_identity = cmatrix_t::Identity(choi_state.rows(), choi_state.cols());

			//Eigen::ComplexEigenSolver<cmatrix_t> ges;
			Eigen::GeneralizedSelfAdjointEigenSolver<cmatrix_t> ges;
		...
		Eigen::Map<smatrix_t> reshape_all_eigens(final_all_eigens.data(), num_block, block_size);

		std::map<string, svector_t> choi_eigen_values;
		...
		for (auto val : choi_eigs)
		{
			...
			svector_t zero_array = svector_t::Zero(val.second.size());

			svector_t choi_value = svector_t::Map(reshape_all_eigens.row(idx).data(), reshape_all_eigens.row(idx).size());
		...
		for (auto items : choi_eigen_values)
		{
			...
			for (auto i = 0; i < choi_values.size(); ++i)
			{
				...
				{
					cvector_t choi_vector = choi_vecs[choi_qubits].col(i); //choi_vecs[bits][:,idx]
					cmatrix_t outer_result = array_outer(choi_vector); //to_projector result
