...
#ifndef  _AMPLITUDE_ENCODE_H
#define  _AMPLITUDE_ENCODE_H
...
* @note
*/
inline QCircuit amplitude_encode(QVec q, vector<double> data) {
	...
	else
	{
		...
		if (sum_0 > 1e-20)
		{
			...
			qcir << X(q[high_bit]) << amplitude_encode(q - temp, vtemp).control({ q[high_bit] }) << X(q[high_bit]);
		}
		if (sum_1 > 1e-20)
		{
			QVec temp({ q[high_bit] });
			vector<double> vtemp(data.begin() + (data.size() >> 1), data.end());
			qcir << amplitude_encode(q - temp, vtemp).control({ q[high_bit] });
		}
	}
	return qcir;
}
