...
void QProgStored::handleQGateWithOneAngle(AbstractQGateNode * gate)
{
    ...
    auto angle = dynamic_cast<QGATE_SPACE::AbstractSingleAngleParameter *>(gate->getQGate());
    if (nullptr == angle)
    {
...
void QProgStored::handleQGateWithFourAngle(AbstractQGateNode * gate)
{
	...
	auto angle = dynamic_cast<QGATE_SPACE::AbstractAngleParameter *>(gate->getQGate());
	if (nullptr == angle)
	{
		QCERR("get angle error");
		throw invalid_argument("get angle error");
    ...
    alpha = angle->getAlpha();
    beta = angle->getBeta();
    gamma = angle->getGamma();
    delta = angle->getDelta();
...
void QProgStored::transformQProgByTraversalAlg(QProg *prog)
{
	...
	execute(prog->getImplementationPtr(), nullptr);
}
...
void QPanda::convert_qprog_to_binary(QProg &prog, QuantumMachine *qm, const string &filename)
{
	transformQProgToBinary(prog, qm, filename);
}

std::vector<uint8_t> QPanda::convert_qprog_to_binary(QProg & prog, QuantumMachine *qm)
{
	return transformQProgToBinary(prog, qm);
}