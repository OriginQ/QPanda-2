...
std::vector<GenericStringRef<char>> slsqp_save_para_name = { "work", "w", "jw",
"nevals_p","niters_p",
"t", "f0", "h1", "h2", "h3", "h4","n1", "n2", "n3", "t0", "gs","tol", "line",
"alpha", "iexact", "incons", "ireset", "itermx", "x0",
"iter", "want_grad", "mode", "prev_mode", "fcur", "fprev" };

void restore_break(std::string save_file_name, slsqpb_state* state,
    double* work, double* w, int* jw, int* nevals_p, int* niters_p, 
    int work_length, int len_w, int len_jw,
    int para_num, ...)
{
    Document jsonDoc;
    ...
    for (i = 0; i < para_number - 2; i++)
    {
        *(va_arg(pointer_para, int*)) = (jsonDoc.FindMember(
            slsqp_save_para_name[i + 24])->value).GetDouble();
    }
    *(va_arg(pointer_para, double*)) = (jsonDoc.FindMember(
        slsqp_save_para_name[24 + para_number - 2])->value).GetDouble();
    *(va_arg(pointer_para, double*)) = (jsonDoc.FindMember(
        slsqp_save_para_name[24 + para_number - 1])->value).GetDouble();
    va_end(pointer_para);

    int length[3] = { work_length, len_w, len_jw };
    ...
    {
        jw[i] = jsonDoc["jw"][i].GetDouble();
    }

    *nevals_p = jsonDoc["nevals_p"].GetDouble();
    *nevals_p = jsonDoc["niters_p"].GetDouble();
    state->t = jsonDoc["t"].GetDouble();
    state->f0 = jsonDoc["f0"].GetDouble();
    state->h1 = jsonDoc["h1"].GetDouble();
    state->h2 = jsonDoc["h2"].GetDouble();
    state->h3 = jsonDoc["h3"].GetDouble();
    state->h4 = jsonDoc["h4"].GetDouble();
    state->n1 = jsonDoc["n1"].GetDouble();
    state->n2 = jsonDoc["n2"].GetDouble();
    state->n3 = jsonDoc["n3"].GetDouble();
    state->t0 = jsonDoc["t0"].GetDouble();
    state->gs = jsonDoc["gs"].GetDouble();
    state->tol = jsonDoc["tol"].GetDouble();
    state->line = jsonDoc["line"].GetDouble();
    state->alpha = jsonDoc["alpha"].GetDouble();
    state->iexact = jsonDoc["iexact"].GetDouble();
    state->incons = jsonDoc["incons"].GetDouble();
    state->ireset = jsonDoc["ireset"].GetDouble();
    state->itermx = jsonDoc["itermx"].GetDouble();
    double tmp = (jsonDoc["x0"].GetDouble());
    state->x0 = &tmp;
}

void save_break(std::string save_file_name, slsqpb_state state,
    double* work, double* w, int* jw, int nevals_p, int niters_p,
    int work_length, int len_w, int len_jw, int para_num, ...)
{
    long long para_number = para_num, i;
    ...
    // 数组指针读取数据并写入work,w,jw
    Value work_array(kArrayType);
    for (i = 0; i < work_length; i++)
    {
        work_array.PushBack(work[i], allocator);
    }
    jsonDoc.AddMember(slsqp_save_para_name[0], work_array, allocator);

    Value w_array(kArrayType);
    for (i = 0; i < len_w; i++)
    {
        w_array.PushBack(w[i], allocator);
    }
    jsonDoc.AddMember(slsqp_save_para_name[1], w_array, allocator);

    Value jw_array(kArrayType);
    for (i = 0; i < len_jw; i++)
    {
        jw_array.PushBack(jw[i], allocator);
    }
    jsonDoc.AddMember(slsqp_save_para_name[2], jw_array, allocator);

    double temp_state[19] = { state.t, state.f0, state.h1, state.h2, state.h3,
        state.h4, (double)state.n1, (double)state.n2, (double)state.n3,
        state.t0, state.gs, state.tol, (double)state.line, state.alpha,
        (double)state.iexact, (double)state.incons, (double)state.ireset,
        (double)state.itermx, *(state.x0) };

    for (i = 0; i < 19; i++) 
    {
        jsonDoc.AddMember(slsqp_save_para_name[i + 5], temp_state[i], allocator);
    }
    ...
    for (i = 0; i < para_number - 2; i++)
    {
        jsonDoc.AddMember(slsqp_save_para_name[i + 24], va_arg(const_para, int), allocator);
    }
    jsonDoc.AddMember(slsqp_save_para_name[24 + para_number - 2], va_arg(const_para, double), allocator);
    jsonDoc.AddMember(slsqp_save_para_name[24 + para_number - 1], va_arg(const_para, double), allocator);
    va_end(const_para);

    FILE* fpout = fopen(save_file_name.c_str(), "wb"); // 非Windows平台使用"w" 
...
nlopt_result nlopt_slsqp(unsigned n, nlopt_func f, void *f_data,
	...
	nlopt_stopping *stop, bool restore_flag,
...
#define U(n) ((unsigned) (n))
    int work_length = U(mpi1) * (n + 1) + U(mpi) + n + 1 + n + n + max_cdim * n;
    work = (double *) malloc(sizeof(double) * (U(mpi1) * ((int64_t)n + 1)  // cgrad = 6
					+ U(mpi)                        // c = 2
					+ n+1 + n + n + max_cdim* (int64_t)n      // grad + xcur + xprev + cgradtmp = 3 + 2 +2 + 2
					+ U((int64_t)len_w))                     // w = 133
			    + sizeof(int) * U(len_jw));         // jw = 7
    if (!work) return nlopt_result::NLOPT_OUT_OF_MEMORY;
    cgrad = work;
    c = cgrad + U(mpi1) * ((int64_t)n + 1);
    grad = c + mpi;
    xcur = grad + n+1;
    xprev = xcur + n;
    cgradtmp = xprev + n;
    w = cgradtmp + max_cdim * (int64_t)n;
    jw = (int *) (w + len_w);
    ...
    if (restore_flag)
    {
        ...
        {
            restore_break(save_file_name, &state, work, w, jw , stop->nevals_p, stop->niters_p, work_length,
                len_w, len_jw, para_length, &iter, &want_grad, &mode,
                &prev_mode, &fcur, &fprev);
        }
        else
        {
    ...
    ++* (stop->niters_p);
    save_break(save_file_name, state, work, w, jw, *stop->nevals_p, *stop->niters_p, work_length, len_w, len_jw,
        para_length, iter, want_grad, mode, prev_mode, fcur, fprev);
	...
	case 1:{ /* don't need grad unless we don't have it yet */
	    ...
	    fcur = f(n, xcur, newgrad, f_data, *(stop->niters_p), *(stop->nevals_p));
	    ++ *(stop->nevals_p);
	    if (nlopt_stop_forced(stop)) {
		fcur = HUGE_VAL; ret = nlopt_result::NLOPT_FORCED_STOP; goto done; }
		...
		for (i = 0; i < p; ++i) {
		    ...
		    if (nlopt_stop_forced(stop)) {
			ret = nlopt_result::NLOPT_FORCED_STOP; goto done; }
		    for (k = 0; k < h[i].m; ++k, ++ii) {
			infeasibility_cur =
			    MAX2(infeasibility_cur, fabs(c[ii]));
			feasible_cur =
			    (int)(feasible_cur && fabs(c[ii]) <= h[i].tol[k]);
			if (newcgrad) {
			    for (j = 0; j < n; ++ j)
		...
		for (i = 0; i < m; ++i) {
			...
			feasible_cur =
			    (int)(feasible_cur && c[ii] <= fc[i].tol[k]);
			if (newcgrad) {
			    for (j = 0; j < n; ++ j)
				cgrad[j*U(mpi1) + ii] = -cgradtmp[k*n + j];
			}
		...
		ret = nlopt_result::NLOPT_ROUNDOFF_LIMITED; /* usually why deriv>0 */
		if (feasible_cur) {
		...
		ret = nlopt_result::NLOPT_ROUNDOFF_LIMITED;
		goto done;
		...
		ret = nlopt_result::NLOPT_FAILURE;
		goto done;
		...
		ret = nlopt_result::NLOPT_INVALID_ARGS;
		goto done;
	}
	prev_mode = mode;
	...
	if (nlopt_stop_evals(stop)) ret = nlopt_result::NLOPT_MAXEVAL_REACHED;
    else if (nlopt_stop_iters(stop)) ret = nlopt_result::NLOPT_MAXITER_REACHED;
    ...
    } while (ret == nlopt_result::NLOPT_SUCCESS);
