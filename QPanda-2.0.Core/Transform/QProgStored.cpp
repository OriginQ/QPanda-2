...
void QProgStored::store(const string &filename)
{
    ...
    if (errno_number || !fp)
    {
        throw param_error_exception("fopen file failure", false);
    }
#else
    ...
    for (auto &val : m_data_list)
    {
        ...
        {
            throw param_error_exception("fwrite file failure", false);
        }
    }
...
void QProgStored::traversalQProg(AbstractQuantumProgram *p_prog)
{
    if (nullptr == p_prog)
    {
        throw param_error_exception("pQProg is null", false);
    }
    ...
    if (p_circuit->isDagger())
    {
        for (auto iter = p_circuit->getLastNodeIter(); iter != p_circuit->getHeadNodeIter(); iter--)
        {
            ...
            case NodeType::MEASURE_GATE:
                break;
            default:
                throw param_error_exception("Circuit is error", false);
                break;
            }
...
void QProgStored::traversalQControlFlow(AbstractControlFlowNode *p_controlflow)
{
    ...
    {
        throw param_error_exception("pQControlFlow is null", false);
    }

    ClassicalCondition *p_classcical_condition = p_controlflow->getCExpr();
    CExpr *p_cexpr = p_classcical_condition->getExprPtr();
    traversalCExpr(p_cexpr);
    ...
    case NodeType::WHILE_START_NODE:
        ...
        break;
    default:
        throw param_error_exception("NodeType is error", false);
        break;
    }
...
void QProgStored::traversalQIfProg(AbstractControlFlowNode *p_controlFlow)
{
    ...
    {
        throw param_error_exception("Node count is out of range", false);
    }
...
void QProgStored::traversalQGate(AbstractQGateNode *p_gate)
{
    if (nullptr == p_gate)
    {
        throw param_error_exception("pQGate is null", false);
    }
    ...
    {
        throw param_error_exception("do not support this gateType", false);
    }
    ...
    {
        throw param_error_exception("pQGate is illegal", false);
    }
    ...
    case GateType::SQISWAP_GATE:
        ...
        break;
    default:
        throw param_error_exception("do not support this type gate", false);
        break;
    }
    ...
    if (m_node_counter > kUshortMax)
    {
        throw param_error_exception("QNode count is out of range", false);
    }
    type_and_number |= (m_node_counter << kCountMoveBit);
        ...
        || GateType::CPHASE_GATE == gate_type)
    {
        ...
        if (nullptr == angle)
        {
            throw param_error_exception("get angle error", false);
        }
...
void QProgStored::traversalQMeasure(AbstractQuantumMeasure *p_measure)
{
    ...
    {
        throw param_error_exception("p_measure is null", false);
    }
    ...
    {
        throw param_error_exception("QBit number is out of range", false);
    }
    qubit_array[0] = (ushort_t)qubit_addr;

    if (cbit_number > kUshortMax)
    {
        throw param_error_exception("QCit number is out of range", false);
    }
    ...
    if (m_node_counter > kUshortMax)
    {
        throw param_error_exception("QNode counter is out of range", false);
    }
    type_and_number |= (m_node_counter << kCountMoveBit);
    m_data_list.emplace_back(pair<uint_t, DataNode>(type_and_number, qubit_data));
...
void QProgStored::traversalQNode(QNode *p_node)
{
    ...
    case NodeType::NODE_UNDEFINED:
        throw param_error_exception("NodeType UNDEFINED", false);
        break;
    default:
        throw param_error_exception("p_node is error", false);
        break;
    }
...
void QProgStored::traversalCExpr(CExpr *p_cexpr)
{
    ...
    if (nullptr == p_cexpr->getCBit())
    {
        ...
        {
            throw param_error_exception("pCExpr is error", false);
        }
...
void qProgBinaryStored(QProg &prog, const string &filename)
{
