...
QuantumVolume::QuantumVolume(MeasureQVMType type, QuantumMachine* qvm)
{
	...
	m_qvm_type = type;
	if (m_qvm_type == MeasureQVMType::ORIGIN_REAL_CHIP)
		m_qcm = dynamic_cast<QCloudMachine*>(qvm);
	else
		m_noise_qvm = dynamic_cast<NoiseQVM*>(qvm);
}

QuantumVolume::QuantumVolume(QCloudTaskConfig config)
{
    m_qvm = new CPUQVM();
    m_qvm->init();
    m_qvm->setConfigure({ 300, 300 });
...
void QuantumVolume::createQvCircuits(std::vector<std::vector<int> > qubit_lists, 
    int ntrials,
	std::vector<std::vector <QvCircuit> >&circuits, 
    std::vector<std::vector <QvCircuit> >&circuits_nomeas)
{
...
void QuantumVolume::createQvCloudCircuits(std::vector<std::vector<int> > qubit_lists,
    int ntrials,
    std::vector<std::vector <QvCircuit> >&circuits,
    std::vector<std::vector <QvCircuit> >&circuits_nomeas)
{
    ...
    for (int trial = 0; trial < ntrials; trial++)
    {
        ...
        for (int depthidx = 0; depthidx < m_depth_list.size(); depthidx++)
        {
            ...
            std::vector<ClassicalCondition> cr;
            for (int i = 0; i < q_max + 1; i++)
                qr.push_back(m_qcloud.allocateQubitThroughPhyAddress(i));

            for (int i = 0; i < depth; i++)
            {
                qr2.push_back(m_qvm->allocateQubitThroughPhyAddress(i));
                cr.push_back(m_qcloud.cAlloc(i));
            }
...
void QuantumVolume::calcIRealChipResult(std::vector<std::vector <QvCircuit> >&circuits, int shots)
{
    m_shots = m_cloud_config.shots;
    for (int trial = 0; trial < m_ntrials; trial++)
    {
        for (int depthidx = 0; depthidx < circuits[trial].size(); depthidx++)
        {
            ...
            auto tmp_res = m_qcloud.real_chip_measure(prog,
...
size_t QuantumVolume::calcQuantumVolume(const std::vector<std::vector<int>> &qubit_lists,
	int ntrials, int shots, QMachineType type)
{
	vector<vector <QvCircuit >> cirs;
	vector<vector <QvCircuit >>cirs_nomea;

    if (type == QMachineType::NOISE)
    {
    ...
    {
        createQvCloudCircuits(qubit_lists, ntrials, cirs, cirs_nomea);
        calcIdealResult(cirs_nomea);
        calcIRealChipResult(cirs, shots);
    }
...
{
	QuantumVolume qv(MeasureQVMType::ORIGIN_REAL_CHIP, qvm);
	return qv.calcQuantumVolume(qubit_lists, ntrials, shots);
}

size_t QPanda::calculate_quantum_volume(QCloudTaskConfig config, std::vector <std::vector<int> >qubit_lists, int ntrials)
{
    QuantumVolume qv(config);
    return qv.calcQuantumVolume(qubit_lists, ntrials, config.shots, QMachineType::QCloud);
}
