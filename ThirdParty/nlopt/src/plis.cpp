    ...
    std::string cache_file, int m = 20)
{
    ...
    for (i = 0; i < offset_1; i++)
    {
        ...
        work[i + n * 4 + offset_1] = jsonDoc["work"][i + offset + offset_1].GetDouble();
    }
    // uo, vo
    offset += offset_1 * 2;
    ...
    std::string cache_file, int m = 20)
{
    ...
    // m = max number of lower-memory BFGS limit, 3 \le m \le 20
    //int64_t length[3] = { n, mf * n, mf };
    int64_t offset_1 = MAX2(n, n * mf), offset_2 = MAX2(n, mf);
    ...
    for (i = 0; i < offset_2; i++)
    {
        ...
        work_array.PushBack(work[i + n * 4 + offset_1 * 2 + offset_2], allocator);
    }
    ...
    bool restore_flag, std::string save_file_name)
{
    ...
    *f = objgrad(*nf, &x[1], &gf[1], objgrad_data, *(stop->niters_p), *(stop->nevals_p));
    ++* (stop->nevals_p);
    ++stat_1->nfg;
