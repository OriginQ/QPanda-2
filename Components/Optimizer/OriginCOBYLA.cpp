...
const std::string COBYLA_CACHE_HEADER = "cobyla_break_point.json";

namespace QPanda
{
    OriginCOBYLA::OriginCOBYLA()
    {
        m_dimension = 0;
        m_fcalls = 0;
        m_iter = 0;
    ...
    void OriginCOBYLA::set_lower_and_upper_bounds(vector_d& lower_bound, vector_d& upper)
    {
        nlopt_set_lower_bounds(&opter, lower_bound.data());
        nlopt_set_upper_bounds(&opter, upper.data());
    }

    void OriginCOBYLA::add_equality_constraint(QOptFunc func)
    {
    ...
    void OriginCOBYLA::add_inequality_constraint(QOptFunc func)
    {
        nlopt_func a([func](unsigned n, const double* x, double* gradient, void* func_data)
            {
                vector_d x_data(x, x + n), grad_data(gradient, gradient + n);
    ...
    void OriginCOBYLA::init() {
        ...
        opter = nlopt_create(nlopt_algorithm::NLOPT_LN_COBYLA, m_dimension);
    ...
    void OriginCOBYLA::outputResult()
    {
